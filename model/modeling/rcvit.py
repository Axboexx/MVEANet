...
class RCViT(nn.Module):
    def __init__(self, layers, embed_dims, mlp_ratios=4, downsamples=[True, True, True, True],
                 norm_layer=nn.BatchNorm2d, attn_bias=False,
...
def rcvit_l(**kwargs):
    model = RCViT(
        layers=[4, 4, 8, 4], embed_dims=[96, 128, 256, 512], mlp_ratios=4, downsamples=[True, True, True, True],
        norm_layer=nn.BatchNorm2d, attn_bias=False, act_layer=nn.GELU, drop_rate=0., fork_feat=True, **kwargs)
    state_dict = torch.load('/22liuchengxu/MVANet-main/MVANet-main/model/modeling/SFPN-CAS-ViT-T.pth')
...
if __name__ == '__main__':
    model = rcvit_l()
    tensor = torch.randn(5, 3, 512, 512)
    out = model(tensor)
    # out:{
    #     [1,96,128,128]
    #     [1,128,64,64]
